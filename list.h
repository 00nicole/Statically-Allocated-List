...
struct Node_s {
    void* data; // each node will hold something, no data type defined so its void *
    Node* next; // node->next points to next node
    Node* prev; // node->prev points to previous node 
    int index; // holds the index of the nodearray in which the node is kept.
    //Node* node_head;
    bool avail;
};
...
struct List_s{
    ...
    Node* currentpointer; // current item
    int index; // holds the index of the headarray in which the head is kept.
    int size; // counts how many nodes are in the list
    //Node* list_head; // holds the head of the list in the free_lists array;
